...
// Staff Calendar APIs
export async function getStaffCalendarSummaryApi(params?: {
  month?: number; // 0-11
  year?: number;
  status?: string;
  priority?: string;
  ...
  const url = `${API_BASE}/stats/complaints/calendar/staff-summary${qs({
    month: typeof params?.month === "number" ? params?.month : undefined,
    year: typeof params?.year === "number" ? params?.year : undefined,
    status: params?.status,
    priority: params?.priority,
...
export async function getStaffCalendarDayApi(params: {
  date: string; // yyyy-mm-dd
  viewType?: "submission" | "deadline";
  status?: string;
  priority?: string;
  ...
  const url = `${API_BASE}/stats/complaints/calendar/staff-day${qs({
    date: params?.date,
    viewType: params?.viewType,
    status: params?.status,
    priority: params?.priority,
